...
export default function Candidate() {
  ...
  const candidateList = (data?.items as unknown as ExtendedCandidateListItem[]) || []
  ...
  const columns = [
        ...
        return (
          <div className="flex flex-wrap gap-1 max-w-[250px]">
            {skills.slice(0, 3).map((skill) => (
              <span key={skill} className="px-1.5 py-0.5 rounded text-[11px] bg-secondary border text-secondary-foreground">
                {skill}
              </span>
            ))}
            {skills.length > 3 && (
              <span className="text-xs text-muted-foreground self-center">+{skills.length - 3}</span>
            )}
          </div>
        )
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className={`capitalize px-2 py-0.5 rounded-full text-xs font-medium border ${styles[status] || styles.new}`}>
            {status}
          </span>
        )
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col gap-0.5">
            <span className={`w-fit px-1.5 py-0.5 rounded text-[11px] font-medium border ${
              status === "active" 
  ...
  return (
    <div className="p-6 space-y-4 w-full">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight">Candidates Pipeline</h1>
        <p className="text-sm text-muted-foreground">Monitor technical assessments and candidate lifecycle tracks.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="w-full border border-destructive/20 rounded-lg p-6 bg-destructive/5 text-center flex flex-col items-center justify-center gap-3">
          <div className="text-destructive font-medium">Failed to load candidates</div>
          <p className="text-sm text-muted-foreground max-w-md">
            {error instanceof Error ? error.message : "An unexpected network error occurred. Please try again."}
          </p>
          <button 
            onClick={() => refetch?.()}
